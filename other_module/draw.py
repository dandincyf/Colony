...
def show_animation(locus,follow_locus,motion_mana):
    ...
    plt.plot(motion_mana.motion_cal.map.goal[0], motion_mana.motion_cal.map.goal[1], "ro") #画目标
    #主机坐标
    x = []
    y = []
    #僚机坐标
    xs = [[] for i in range(9)]
    ys = [[] for i in range(9)]
    plt.plot(motion_mana.motion_cal.uav_main.uav[-1].x, motion_mana.motion_cal.uav_main.uav[-1].y, "k.") #画主机
    follow_num = 0
    for i in follow_locus: #画僚机1-9
